...
### [05. 用两个栈实现队列](https://leetcode-cn.com/problems/yong-liang-ge-zhan-shi-xian-dui-lie-lcof/)
...
&#8195;其实这里只要了解栈和队列的最大的不同点即可：

- 栈：后进先出，只能在栈顶入栈和出栈元素
- 队列：先进先出，只能在队列的头入队和队尾出队元素

&#8195;根据两者的特性，可以发现，**当我们把一个栈（stack1）的元素出栈后再入另一个栈（stack2），则stack2的出栈顺序就和队列的出队列顺序相同。**

&#8195;所以，我们引入二个栈，第一个栈用于维护入队列所有的元素，用第二个栈维护待删除的元素，在执行删除操作的时候我们首先看下第二个栈是否为空。如果为空，我们将第一个栈里的元素一个个弹出插入到第二个栈里，这样第二个栈里元素的顺序就是待删除的元素的顺序，要执行删除操作的时候我们直接弹出第二个栈的元素返回即可。

#### 5.3 题解代码（逐行解释版）：

- [Python](https://github.com/moonlighf/AlgorithmWithLeetCode/blob/master/DataStructure/03.Stack/Python/Offer09_CQueue.py)
